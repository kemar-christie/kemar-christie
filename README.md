# Hi there 👋, Welcome!

![FRONT OF CARD](https://github.com/user-attachments/assets/c8c9fe62-cad5-40e7-90c6-2bb5ed0a2338)


Hello! My name is **Kemar Christie**. I am an aspiring Machine Learning Engineer passionate about combining engineering principles and cloud infrastructure to build innovative AI solutions.

---

### About Me
- 🌐 I am currently located in **Kingston, Jamaica** and attend the **University of Technology, Jamaica**, pursuing a BSc in Computer Science.
- 💡 I enjoy exploring cutting-edge **AI technologies** and expanding knowledge in **machine learning**.
- ✏️ I Enjoy researching new technologies, listening to tech podcasts, and diving into the world of AI development.
- 🤝  I am Passionate about mentoring and guiding youth in tech.

---

### Current Focus
- 🚀 Building expertise in **Machine Learning** and **Cloud Infrastructure**.
- 🔧 Enhancing skills in **Python**, **SQL**, **Java**, **TensorFlow**, **PyTorch**, **GCP**, **AWS**, **AZURE AI**, **DOCKER** & **GIT**.
- 🎨 Working on real-world projects to solve problems using **AI-powered solutions**.

---

### Skills
- **Programming Languages:** Python, SQL, java
- **AI/ML Tools:** TensorFlow, PyTorch
- **Cloud Platforms:** AWS, Azure, Google Cloud
- **Others:** Git, Docker

---

### Fun Facts About Me
- 🔊 I love listening to tech podcasts for daily inspiration, technological advancements and trends.
- 🚨 My dream is to bridge the gap between **AI innovation** and **accessible tech solutions**.
- ☀️ You can call me "Kem"!

---

### Connect with Me
Feel free to reach out and collaborate:
- [LinkedIn](https://www.linkedin.com/in/your-profile/)  👤
- [Twitter](https://twitter.com/your-handle)  🕊️
- [GitHub](https://github.com/your-github)  💻

---

### 🎭 Projects & Highlights
- **[AI-Enabled-GPA-System](https://github.com/project-link)**: A Python & Prolog AI Enabled GPA System.
- **[Event-Sceduling-System](https://github.com/project-link)**: SQL & JAVA enabled Event Scheduling System.

Thank you for stopping by! Let's connect and build something awesome together. 🚀
